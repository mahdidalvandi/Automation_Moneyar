            ...
            ExcelRenderer(fileObj, (err, resp) => {
                if (err) {
                    ...
                    for (let j = 1; j < resp.rows.length; j++) {
                        ...
                        if (!findArrayElementByTitle(contractTypeMethods, resp.rows[j][11])) {
                            errorObjects['contractType'] = 'خطا در نوع قرارداد';
                        }
                        if (!findArrayElementByTitle(marriageStatusTypeMethods, resp.rows[j][4])) {
                        ...
                        userDataBuf = [...userDataBuf, {
                            index: j, firstName: resp.rows[j][0], lastName: resp.rows[j][1], fatherName: resp.rows[j][2] ? resp.rows[j][2] : null
                            , gender: resp.rows[j][3] ? resp.rows[j][3] : null, marriageStatus: resp.rows[j][4] ? resp.rows[j][4] : null
                            , nationalCode: resp.rows[j][5], birthDate: resp.rows[j][6] ? resp.rows[j][6] : null, mobile: resp.rows[j][7], education: resp.rows[j][8] ? resp.rows[j][8] : null
                            , email: resp.rows[j][9] ? resp.rows[j][9] : null, entryDate: resp.rows[j][10] ? resp.rows[j][10] : null, contractType: resp.rows[j][11] ? resp.rows[j][11] : null, error: errorObjects
                        }];
    ...
    const onSubmit = async (event) => {
        ...
        for (let i = 0; i < userData.length; i++) {
            if (userData[i].error.gender) {
                object['data'] = [...object['data'], `خطای جنسیت در ردیف ${i + 1} `, <br />];
                hasError = true;
            }
            if (userData[i].error.contractType) {
                object['data'] = [...object['data'], `خطای نوع قرارداد در ردیف ${i + 1} `, <br />];
                hasError = true;
            }
            if (userData[i].error.marriageStatus) {
                object['data'] = [...object['data'], `خطای وضعیت تاهل در ردیف ${i + 1} `, <br />];
                hasError = true;
            }
            if (userData[i].error.educations) {
                object['data'] = [...object['data'], `خطای مدرک تحصیلی در ردیف ${i + 1} `, <br />];
                hasError = true;
            }
            if (userData[i].error.entryDate) {
                object['data'] = [...object['data'], `خطای تاریخ ورود به شرکت در ردیف ${i + 1} `, <br />];
                hasError = true;
            }
            if (userData[i].error.birthDate) {
                object['data'] = [...object['data'], `خطای تاریخ تولد در ردیف ${i + 1} `, <br />];
                hasError = true;
            }
            if (userData[i].error.contractType) {
                object['data'] = [...object['data'], `خطای نوع قرارداد در ردیف ${i + 1} `, <br />];
            ...
            userData[i].gender = userData[i].gender ? findArrayElementByTitle(genderTypeMethods, userData[i].gender) ? findArrayElementByTitle(genderTypeMethods, userData[i].gender).id : null : null
            userData[i].marriageStatus = userData[i].marriageStatus ? findArrayElementByTitle(marriageStatusTypeMethods, userData[i].marriageStatus) ? findArrayElementByTitle(marriageStatusTypeMethods, userData[i].marriageStatus).id : null : null
            ...
            <div className="md:pr-52 flex flex-col flex-1">
                ...
                {!currentUserActions ? null : CheckIfAccessToPage(window.location.pathname) ?
                    <main>
                        ...
                        <BatchUsersTable loadingData={false} data={userData} roleData={currentUserRole} />
