...
export default function UserDetails() {
    ...
    const { getUserDetails, userDetailsData, isUserDetailsLoading } = useUserDetails();
        ...
        if (userDetailsData && userDetailsData.users) {
            ...
            if (val.length > 2) {
                ...
                for (let i = 0; i < userDetailsData.users.length; i++) {
                    if (userDetailsData.users[i].first_name.includes(val) || userDetailsData.users[i].last_name.includes(val)) {
                        ...
                        <div className="py-6">
                                ...
                                <div className="bg-white px-4 py-5 border-b border-gray-200 sm:px-6">
                                    <div className="-ml-4 -mt-2 flex items-center justify-between flex-wrap sm:flex-nowrap">
                                        <div className="ml-4 mt-2">
                                            <h2 className="text-lg leading-6 font-large text-gray-900">لیست کاربران شرکت {isUserDetailsLoading ? '' : userDetailsData.company_name}</h2>
                                        </div>
                            ...
                            <div className="w-full px-4 sm:px-6 md:px-8">
                                {currentUserRole && !isUserDetailsLoading ?
                                    <UserDetailsTable loadingData={false} data={searchHasValue ? searchData : userDetailsData.users} roleData={currentUserRole} /> : null}
                            </div>
                        </div>
                    </main> : <Forbidden />}
            </div>
