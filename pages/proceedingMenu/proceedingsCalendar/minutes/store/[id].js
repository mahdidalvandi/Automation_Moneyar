...
export default function SubStoreCalendar() {
  ...
  useEffect((_) => {
    axios
      .get(`/api/v1/calendar/get/${eventUUID}`)
      .then((res) => {
        setSelectedEvent(res.data.data[0]);
        setDate(res.data.data[0].timestamp);
        setDate2(moment.unix(res.data.data[0].timestamp).add(3600000).unix());
        setAgenda(res.data.data[0].agenda);
        setPlace(res.data.data[0].place);
        setTitle(res.data.data[0].title);
        setRecipient(res.data.data[0].attend_items);
        setAuthorUUID(res.data.data[0].author_data);
        setAuthorData(res.data.data[0].author_data);
      })
      .catch((error) => {
        //if (error.response.status != 409) throw error;
      });
  ...
  const onSubmit = async (event) => {
    ...
    if (String(selectedEvent.timestamp) >= String(date2)) {
      setErrors(["زمان شروع بیشتر از زمان پایان می‌باشد"]);
      return;
    }
    axios
      .post("/api/v1/calendar/minutes/add", {
        event_uuid: eventUUID,
        author_uuid: authorUUID.uuid,
        start_at: String(selectedEvent.timestamp),
        end_at: String(date2),
        title,
        subject,
        agenda: agenda,
              ...
              <div>
                <div className="flex justify-between py-5 items-center">
                  <div className="sm:col-span-6">
                    تاریخ تنظیم صورت‌جلسه: {moment().format("jYYYY/jMM/jDD")}
                  </div>
                </div>
                {authorData ? (
                  <div className="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                    ...
                    <div>
                      ...
                      <DatePicker
                        ...
                        calendar={persian}
                        locale={persian_fa}
                        plugins={[<TimePicker key="02" position="bottom" />]}
                        placeholder="انتخاب کنید.."
                        calendarPosition="bottom-right"
                        inputclassName="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-amber-500 focus:border-amber-500 sm:text-sm"
                        containerStyle={{
                        ...
                        <Autocomplete
                          ...
                          renderInput={(params) => (
                            <TextField
                              ...
                              variant="standard"
                              placeholder="افزودن .."
                              value={authorData ? authorData.full : ""}
                              onChange={(event) =>
                    ...
                    <div className="sm:col-span-3">
                      <Textarea
                        ...
                        name={sessionDesc}
                        rows="1"
                        onChange={(event) => setSessionDesc(event.target.value)}
                    ...
                    <div className="sm:col-span-3">
                      <Textarea
                        ...
                        rows="1"
                        onChange={(event) =>
                          setUpcomingAgenda(event.target.value)
                        }
                      ...
                      <table className="font-light table-auto w-full text-sm font-light my-3 shadow rounded-md overflow-hidden">
                        <thead className="bg-gray-100">
                          <tr>
                            ...
                            <th className="text-right">شرح اقدام</th>
                            <th>اقدام کننده</th>
                            <th>نام مسئول</th>
                            <th>مهلت زمان بررسی</th>
                            <th></th>
                          </tr>
                        </thead>
                        <tbody>
                              ...
                              <tr key={index}>
                                ...
                                <td>
                                  <input
                                    ...
                                    onChange={(e) =>
                                      changeSharhEghdam(index, e.target.value)
                                    }
                    ...
                    <div className="sm:col-span-6 border-t border-gray-300 py-5">
                      <div className="flex justify-between items-center">
                        <p className="w-40 text-sm ml-3">
                          اسامی و امضای حاضرین*
                        </p>
                      ...
                      <table className="font-light table-auto w-full text-sm font-light my-3 shadow rounded-md overflow-hidden">
                        <thead className="bg-gray-100">
                          <tr>
                            ...
                            <th className="text-right">سمت</th>
                          </tr>
                        </thead>
                        <tbody>
                      ...
                      <div className="space-x-5 space-x-reverse ">
                        <label>
                          <input
                            ...
                            onClick={(_) => setConfidentiality(2)}
                            checked={confidentiality == 2}
                          />
                        ...
                        <label>
                          <input
                            type="radio"
                            name="confidentiality"
                            onClick={(_) => setConfidentiality(1)}
                            checked={confidentiality == 1}
                          />
                        ...
                        <label>
                          <input
                            type="radio"
                            name="confidentiality"
                            onClick={(_) => setConfidentiality(0)}
                            checked={confidentiality == 0}
                          />
                  ...
                  <div className="grid-cols-6 flex justify-center m-10 ">
                    <Image
                      alt="image"
                      src="/images/dots.svg"
                      height={22}
