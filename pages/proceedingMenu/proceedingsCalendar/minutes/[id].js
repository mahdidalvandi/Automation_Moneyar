...
export default function SubStoreCalendar(props) {
    ...
    function getEventInfo() {
        axios
            .get(`/api/v1/calendar/minutes/info/${eventUUID}`)
            .then((res) => {
                setEvent(res.data.data)
                setDate(moment.unix(res.data.data.start_at).format("YYYY/MM/DD HH:mm:ss"))
    ...
    function checkIfSigner() {
        if (event.attends) {
            for (let i = 0; i < event.attends.length; i++) {
                if (event.attends[i].user_uuid == user.uuid && !event.attends[i].is_approved) {
                    return true
                }
            }
        }
            ...
            <div className="md:pr-52 flex flex-col flex-1 mb-20">
                ...
                <div className="w-full mx-auto px-4 sm:px-6 md:px-8 grid grid-cols-2 gap-6 py-10">
                    ...
                    <div className="flex flex-col space-y-2">
                        ...
                        <p className="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-amber-500 focus:border-amber-500 sm:text-sm">
                            {event.start_at ? moment.unix(event.start_at).format("HH:mm:00 YYYY/MM/DD") : null}
                        </p>
                    </div>
                    <div className="flex flex-col space-y-2">
                        <p className="block text-sm font-medium  text-gray-700">
                        ...
                        <p className="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-amber-500 focus:border-amber-500 sm:text-sm">
                            {event.end_at ? moment.unix(event.end_at).format("HH:mm:00 YYYY/MM/DD") : null}
                        </p>
                    </div>
                    <div className="flex flex-col space-y-2">
                        <p className="block text-sm font-medium  text-gray-700">
                    ...
                    <div className="flex flex-col space-y-2">
                        ...
                        <p className="text-justify appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-amber-500 focus:border-amber-500 sm:text-sm" style={{ whiteSpace: "pre-line" }}>
                            {event.agenda}
                        </p>
                    </div>
                    <div className="flex flex-col space-y-2">
                    ...
                    <div className="col-span-2">
                        ...
                        <div >
                            <table className="font-light table-auto w-full text-sm font-light my-3 shadow rounded-md overflow-hidden">
                                ...
                                <tbody>
                                    {
                                        event.approvals && event.approvals.map((item, index) => {
                                            return (
                                                <tr key={index}>
                                                    ...
                                                    <td>
                                                        <p className="text-sm w-full border-0 p-2" >{item.sharhEghdam}</p>

                                                    </td>
                                                    <td>
                                                        <input type="text" disabled value={item.eghdamKonande} onChange={e => changeEghdamKonande(index, e.target.value)} className="text-sm text-center w-full border-0 p-2" placeholder="اقدام کننده" />
                                                    </td>
                                                    <td>
                                                        <input type="text" disabled value={item.nameMasol} onChange={e => changeNameMasol(index, e.target.value)} className="text-sm text-center w-full border-0 p-2" placeholder="نام مسئول" />
                                                    </td>
                                                    <td>
                                                        <input type="text" disabled value={item.mohlatZamanBaresi} onChange={e => changeMohlatZamanBaresi(index, e.target.value)} className="text-sm text-center w-full border-0 p-2" placeholder="مهلت زمان بررسی" />
                                                    </td>
                                                </tr>
                                            )
                                        })
                    ...
                    <div className="col-span-2">
                        ...
                        <div >
                            <table className="font-light table-auto w-full text-sm font-light my-3 shadow rounded-md overflow-hidden">
                                ...
                                <tbody>
                                    {
                                        event.attends && event.attends.map((item, index) => {
                                            return (
                                                <tr key={index}>
                                                    <td className="text-center">
                                                        {index + 1}
                                                    ...
                                                    <td>
                                                        <input type="text" disabled value={item.first_name + ' ' + item.last_name} className="text-sm w-full border-0 p-2" placeholder="شرح اقدام" />
                                                    </td>
                                                    <td className="text-center">
                                                        {
                                                            item.is_approved ?
                    ...
                    <div className="sm:col-span-2 text-base">
                        ...
                        <div className="space-x-5 space-x-reverse ">
                            <label>
                                <input type="radio" name="confidentiality" disabled checked={event.confidentiality == 2} />
                                <span className="mr-2">خیلی محرمانه</span>
                            </label>
                            <label>
                                <input type="radio" name="confidentiality" disabled checked={event.confidentiality == 1} />
                                <span className="mr-2">محرمانه</span>
                            </label>
                            <label>
                                <input type="radio" name="confidentiality" disabled checked={event.confidentiality == 0} />
                    ...
                    {
                        checkIfAttend() ? user && event.attends && user.uuid && checkIfSigner() ?
                            (
                ...
                <div className="pt-5 border-t">
                    <div className="flex justify-end ml-4 space-x-3 space-x-reverse">
                        ...
                        {event.length != 0 && CheckIfAccess("edit_minute") ? !event.has_sign ?
                            <Link
                                href={`/proceedingMenu/proceedingsCalendar/minutes/edit/${eventUUID}`}
                            >
                                <span
                                    className="rounded-md mb-4 ml-4 bg-[#ffc107] hover:bg-[#a97e35] cursor-pointer py-2 px-4 text-sm font-medium text-white shadow-sm focus:outline-none ">
                                    ویرایش صورت‌جلسه
                                </span>
                            </Link> :
                            <button
                                className="rounded-md ml-4 mb-4 bg-[#c7c7c7] py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-[#c7c7c7] focus:outline-none ">
                                <span>
                                    ویرایش صورت‌جلسه
                                </span>
                            </button> : null}
