...
function InitialData() {
    ...
    const onSubmit = async (event) => {
        ...
        var object = {};
        if (!post) {
            object['title'] = 'نام سمت الزامی است';
            setErrors(object);
            return;
        }

        const postFormData = new FormData();
        postFormData.append("admin_uuid", manager.uuid);
        postFormData.append("title", post);

        try {
            setLoadingData(true);
            const response = await axios({
                method: "post",
                url: "/api/v1/company/post/new",
                data: postFormData,
            });
                    ...
                    <main>
                        <div className="py-6">
                            ...
                            <div className="w-full px-4 sm:px-6 md:px-8">
                                {currentUserRole ?
                                    <PostsTable data={searchHasValue ? searchData : data} loadingData={loadingData} roleData={currentUserRole} /> : null}
                            </div>
                        </div>
