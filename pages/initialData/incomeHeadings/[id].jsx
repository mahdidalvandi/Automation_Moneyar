...
import { useIncomeHeadings } from "../../../hooks/incomeHeadings";
...
export default function EditPost() {
    ...
    useEffect(() => {
        if (router.isReady) {
            getIncomeHeadings(router.query.id);
        }
    }, [router.isReady]);

    const { getIncomeHeadings, incomeHeadingsData, isPostLoading } = useIncomeHeadings();

    const onSubmit = async (event) => {
        event.preventDefault();
        const mailFormData = new FormData();

        mailFormData.append("title", title ? title : incomeHeadingsData.title),
            mailFormData.append("uuid", incomeHeadingsData.uuid);

        try {
            const response = await axios({
                method: "post",
                url: "/api/v1/company/income_headings/update",
                data: mailFormData,
                headers: { "Content-Type": "multipart/form-data" },
            });
            if (response.data.status == 200) {
                window.location.assign("/initialData/incomeHeadings");
            }
        } catch (error) {
                        ...
                        <div className="py-6">
                            ...
                            <div className="pt-3 pb-2 px-10 border-t border-gray-200">
                                <div className="flex justify-end">
                                    ...
                                    <Link href="/initialData/incomeHeadings">
                                        <button
                                            type="button"
