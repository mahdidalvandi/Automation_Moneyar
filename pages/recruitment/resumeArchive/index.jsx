...
import ResumeArchiveTable from "../../../components/table/resumeArchiveTable";
...
export default function ProceedingsList() {
    ...
    const handleDelete = (h, t) => () => {
        ...
        var selectedHashtagBuf = selectedHashtags.filter((hashtag) => hashtag && hashtag.uuid !== h);
        var selectedHashtagTitlesBuf = selectedHashtagsTitles.filter((hashtag) => hashtag && hashtag !== t);
        setSelectedHashtags(selectedHashtagBuf)
        setSelectedHashtagsTitles(selectedHashtagTitlesBuf)
        searchHashtags(selectedHashtagBuf);
    }
        ...
        if (data) {
            ...
            if (val.length > 2) {
                ...
                for (let i = 0; i < data.length; i++) {
                    if (data[i].applicant_name.includes(val) || data[i].applicant_job_position.includes(val) || data[i].applicant_mobile.includes(val)) {
                    ...
                    <main>
                        <div className="py-6">
                            ...
                            <div className="w-full px-4 sm:px-6 md:px-8">
                                        ...
                                        <div className="relative w-full text-gray-400 border border-gray-300 rounded-r-md shadow-sm focus-within:text-gray-600">
                                            <div className="grid grid-cols-4 mb-2 gap-3">
                                                <Autocomplete
                                                    ...
                                                    onChange={(event, newValue) => { selectHashtags(newValue); }}
                                                    getOptionLabel={(
                                                        person
                                                    ) =>
                                                        person.title
                                                    ...
                                                    {selectedHashtags.map((item, index) => {
                                                        return (
                                                            <div className="ml-1 mr-1">
                                                                {item ?
                                                                    <Chip label={item.title} variant="outlined" onDelete={handleDelete(item.uuid, item.title)} /> : null}</div>
                                                        )
                                                    })}
                                                </Stack>
                                            </div>
                                ...
                                <ResumeArchiveTable data={searchHasValue ? searchData : data} loadingData={false} isArchived={true} roleData={currentUserRole} searchedTag={selectedHashtagsTitles}
                                    setClicked={(per) => {
                                        GetData(per)
                                    }}
                                />
