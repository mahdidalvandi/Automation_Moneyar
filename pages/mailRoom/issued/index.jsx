...
export default function Dashboard() {
      ...
      if (val.length > 2) {
        ...
        for (let i = 0; i < data.length; i++) {
          if (
            ...
            (data[i].secretariat_number &&
              data[i].secretariat_number.includes(toFarsiNumber(val))) ||
            data[i].subject.includes(val) ||
            data[i].to_company.includes(val)
          ) {
  ...
  useEffect(() => {
    async function getData() {
      ...
      await axios
        .get(`/api/v1/mailroom/issued/list?type=0&page=${updatedPINdex}`)
        .then((response) => {
