...
export default function NewEmail() {
  ...
  const printIssuedMail = async (event) => {
    ...
    axios
      ...
      .then((res) => {
        window.open(
          `${res.data.data.url}${withHeader == "0" ? "1" : "0"}`,
          "_blank",
          "noopener,noreferrer"
        );
      })
  ...
  return (
    <div>
              ...
              <div className="max-w-full mx-auto px-4 sm:px-6 md:px-8">
                <div className="border-b border-gray-200">
                  <div className="sm:flex sm:items-baseline justify-between">
                    <h3 className="text-lg text-gray-900">مشاهده نامه صادره</h3>
                    {issuedData.issuance_timestamp ? (
                      <h3 className="text-lg text-gray-900">
                        {`شماره نامه صادره:${issuedData.secretariat_number}`}
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        {`تاریخ صدور نامه:${moment
                          .unix(issuedData.issuance_timestamp)
                  ...
                  <div className="space-y-8 divide-y divide-gray-200">
                    <div>
                      <div className="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                        ...
                        {issuedData && issuedData.actions
                          ? issuedData.actions.map((action, b) => (
                              <div
                                className="col-span-3 rounded-md border p-3"
                                key={b}
                              >
                                <div className="flex justify-between">
                                  <TextWithLabel
                                    title={`${action.first_name} ${action.last_name}`}
                                  />
                                  {action.status_timestamp ? (
                                    <TextWithLabel
                                      title={`زمان عملیات: ${moment
                                        .unix(action.status_timestamp)
                                        .format("HH:mm:ss YYYY/MM/DD")}`}
                                    />
                                  ) : null}
                                </div>
                    ...
                    <div className="flex justify-end">
                      {
                        ...
                        !issuedData.issuance_timestamp ? (
                          ...
                          issuedData.isApproved &&
                          issuedData.issuance_timestamp ? null : null // </button> //     چاپ نامه‌ // > //     className="inline-flex ml-2 justify-center py-2 px-4  shadow-sm text-sm font-medium rounded-md text-white bg-[#43a047] hover:bg-[#2d592f] focus:outline-none " //     onClick={(e) => handleClickOpen(e)} //     //onClick={(e) => printIssuedMail(e)} // <button
                      }
                      <Link
                        href={{
      ...
      >
        <DialogTitle id="alert-dialog-title">{"تنظیمات چاپ"}</DialogTitle>
        <DialogContent>
          <div>
            <div className="sm:col-span-2 mb-5">
              ...
              <fieldset className="mt-2">
                <div className="space-y-4 sm:flex sm:items-center sm:space-y-0 sm:space-x-1">
                  {yesNoTypeMethods.map((yesNoTypeMethods) => (
                    <div
                      key={`yesNo${yesNoTypeMethods.id}`}
                      className="flex items-center"
                    >
                      <label
                        htmlFor={`yesNo${yesNoTypeMethods.id}`}
                        className="ml-3 block text-sm font-medium text-gray-700"
                      >
                        <input
                          id={yesNoTypeMethods.id}
                          name="yesNo"
                          type="radio"
                          defaultChecked={yesNoTypeMethods.id === withHeader}
                          className="focus:ring-amber-500 h-4 w-4 text-amber-600 border-gray-300 ml-2"
                          onChange={(e) => {
                            setWithHeader(e.target.id);
                          }}
                        />
            ...
            <div className="sm:col-span-2">
              ...
              <fieldset className="mt-2">
                <div className="space-y-4 sm:flex sm:items-center sm:space-y-0 sm:space-x-1">
                  {dateDirectionTypeMethods.map((dateDirectionTypeMethods) => (
                    <div
                      key={`genderStatusTypeMethods${dateDirectionTypeMethods.id}`}
                      className="flex items-center"
                    >
                      <label
                        htmlFor={`genderStatusTypeMethods${dateDirectionTypeMethods.id}`}
                        className="ml-3 block text-sm font-medium text-gray-700"
                      >
                        <input
                          id={dateDirectionTypeMethods.id}
                          name="GenderStatusMethod"
                          type="radio"
                          defaultChecked={
                            dateDirectionTypeMethods.id === dateDirection
                          }
                          className="focus:ring-amber-500 h-4 w-4 text-amber-600 border-gray-300 ml-2"
                          onChange={(e) => {
                            setDateDirection(e.target.id);
                          }}
                        />
