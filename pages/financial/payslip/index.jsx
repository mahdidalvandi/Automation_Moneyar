...
export default function AddUser(popUpClick) {
        ...
        if (err) {
          ...
          for (let j = 0; j < resp.rows.length; j++) {
            ...
            userDataBuf = [
              ...
              {
                index: j + 1,
                personalCode: resp.rows[j][0],
                nationalCode: resp.rows[j][1],
                firstName: resp.rows[j][2],
                lastName: resp.rows[j][3],
                monthlyAbsence: resp.rows[j][4],
                effectiveOperation: resp.rows[j][5],
                overTime_H: resp.rows[j][6],
                overTime_M: resp.rows[j][7],
                workHoliday_H: resp.rows[j][8],
                workHoliday_M: resp.rows[j][9],
                nightWork_H: resp.rows[j][10],
                nightWork_M: resp.rows[j][11],
                workDeduction_H: resp.rows[j][12],
                workDeduction_M: resp.rows[j][13],
                shiftWork_H: resp.rows[j][14],
                basicSalary: resp.rows[j][15],
                rightToHousing: resp.rows[j][16],
                childrensRight: resp.rows[j][17],
                overTime_m: resp.rows[j][18],
                workerbon: resp.rows[j][19],
                workHoliday: resp.rows[j][20],
                nightWork: resp.rows[j][21],
                consultingFee: resp.rows[j][22],
                rightGuardianship: resp.rows[j][23],
                severancePay: resp.rows[j][24],
                rightAttraction: resp.rows[j][25],
                rightBoss: resp.rows[j][26],
                shiftWork: resp.rows[j][27],
                otherBenefits: resp.rows[j][28],
                rightToMandate: resp.rows[j][29],
                sumofBenefits: resp.rows[j][30],
                imprest: resp.rows[j][31],
                employeeSocialSecurity: resp.rows[j][32],
                lowtimeWork: resp.rows[j][33],
                employeeSupplementaryInsurance: resp.rows[j][34],
                LifeInsurance: resp.rows[j][35],
                salaryTax: resp.rows[j][36],
                onAccountPayment: resp.rows[j][37],
                deductions: resp.rows[j][38],
                personnelLoan: resp.rows[j][39],
                netPayable: resp.rows[j][40],
                error: errorObjects,
              },
            ];
          }
  ...
  const onSubmit = async () => {
    ...
    for (let i = 0; i < userData.length; i++) {
      if (userData[i].error?.monthlyAbsence) {
        object["data"] = [
          ...object["data"],
          ` خطا در بخش غیبت ماهانه به نام: ${userData[i].lastName} `,
          <br key={i} />,
        ];
        hasError = true;
      }
      if (userData[i].error?.nationalCode) {
        object["data"] = [
          ...object["data"],
          ` خطا در بخش کدملی به نام: ${userData[i].lastName} `,
          <br key={i} />,
        ];
        hasError = true;
      }
  ...
  const submit = () => {
    confirmAlert({
      message:
        " شما در حال ارسال فیش حقوقی هست، آیا از انجام این کار مطمعن هستید؟",
      buttons: [
        {
          label: "بله",
          onClick: onSubmit,
