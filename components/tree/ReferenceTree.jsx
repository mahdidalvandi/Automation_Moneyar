...
function ReferenceTree({ uuid }) {
  ...
  useEffect(() => {
    axios.get(`/api/v1/cartable/reference/${uuid}`).then((res) => {
  ...
  const handleClick = (val) => {
    window.open(
      `${process.env.NEXT_PUBLIC_FRONT_URL}/cartable/${val}`,
      "_blank"
    );
  };
  const renderTree = (nodes, i) => (
    <>
      ...
      {nodes.receivers && Array.isArray(nodes.receivers)
        ? addToExpanded(`node-${nodes.id}`)
        : null}
      <TreeItem
        className="mr-8 mt-1"
        style={{ borderRight: `${i >= 0 ? "1px dashed" : ""}` }}
        key={nodes.id}
        nodeId={`node-${nodes.id}`}
        label={
          <p className={`flex mt-1 mr-2`}>
            <div className={`flex ${nodes.is_current ? "bg-green" : ""}`}>
              {nodes.seen ? (
                <div title="مشاهده شده">
                  <MailOpenIcon
              ...
              {nodes.subject ? (
                <div className="flex">
                  <div title={nodes.date}>
                    <p className="bg-blue-600 text-sm text-white p-1">{` ${
                      nodes.subject ? nodes.subject : ""
                    } `}</p>
                  </div>
  ...
  return chart ? (
    ...
    <Image alt="image" src="/images/dots.svg" height={22} width={22} />
  );
}
