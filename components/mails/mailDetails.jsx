...
function MailDetails(props) {
  ...
  useEffect(
    (_) => {
      if (pdfUUID && !pdfLoading) {
        ...
        setEditStatus(updateStatus);
      }
    },
    [pdfUUID]
  );

  const ShowPreviewPdf = (value, type) => {
    if (type == "pdf") {
      ...
      axios
        ...
        .then((res) => {
          if (type == "pdf") {
            setPdfLetter(window.URL.createObjectURL(res.data));
          }
  ...
  const getUpdates = (event) => {
    ...
    axios
      .post("/api/v1/file/getUpdated", {
        file_uuid: letterUUID,
      })
      .then((res) => {
        var object = {};
        setErrors(object);
  ...
  const editDocFileAndSaveChanges = (event) => {
    event.preventDefault();
    // Make a GET request to the server to fetch the file
    axios
      .post("/api/v1/file/editdoc", {
        file_uuid: letterUUID,
      })
      .then((res) => {
        setEditStatus(true);
      ...
      <div className="max-w-8xl mx-auto px-4 sm:px-6 md:px-8">
        <div className="border-b border-gray-400">
          <div className="grid grid-cols-1 gap-y-2 gap-x-4 mb-2 sm:grid-cols-11">
            ...
            {confidentiality ? (
              <div className="sm:col-span-1">
                <span className="text-sm text-center">محرمانگی:</span>
                <h3 className="text-sm text-gray-500 border border-gray-200 rounded-md text-center bg-gray-300 ">
                  {confidentiality
                    ? confidentialityMethods[confidentiality].title
                    : ""}
                </h3>
              </div>
            ) : null}
            {priority ? (
              <div className="sm:col-span-1">
                ...
                <h3 className="text-sm text-gray-500 border border-gray-200 rounded-md text-center bg-gray-300 ">
                  {priority ? priorityMethods[priority].title : ""}
                </h3>
              </div>
            ) : null}
            {letterNo && letterNo != 0 ? (
          ...
          <div className="flex justify-end mb-2">
            {isMailRoomIssued || isMailRoom ? null : !according ? (
              <button
                type="button"
            ...
            <div>
              <div className="mt-2 grid grid-cols-1 gap-y-2 gap-x-4 sm:grid-cols-6">
                <div className="sm:col-span-4">
                  <TextWithLabel title="فرستنده" text={author} />
                </div>
                ...
                <div className="sm:col-span-4">
                  <TextWithLabel title="به" text={recipient} />
                </div>
                {isMailRoomIssued || isMailRoom ? null : (
                  <div className="sm:col-span-2">
                    <TextWithLabel
                      title="موضوع"
                      text={category ? categoryMethods[category].title : ""}
                    />
                  </div>
                )}
                {isMailRoomIssued || isMailRoom ? null : (
                            ...
                            <div className="flex justify-end mb-3">
                              <div className="flex justify-end mb-3">
                                {editStatus ? (
                                  ...
                                  <button
                                    onClick={editDocFileAndSaveChanges}
                    ...
                    {Object.keys(attachments).length != 0
                      ? attachments.map((file, index) => (
                          ...
                          >
                            <button
                              onClick={() =>
                                ShowPreview(file.uuid, file.type, file.name)
                              }
                              type="button"
                            ...
                            >
                              {file.name ? file.name : "فایل" + "." + file.type}
                            </button>

                            <button
                              onClick={() =>
                                DownloadFile(file.uuid, file.type, file.name)
                              }
                              type="button"
                  ...
                  <div className="mt-2 grid grid-cols-1 gap-y-2 gap-x-4 sm:grid-cols-6">
                      ...
                      <TextWithLabel
                        ...
                        text={
                          regardingData.category
                            ? categoryMethods[regardingData.category].title
                            : ""
                        }
                      />
                    </div>
                    <div className="sm:col-span-4">
                      <TextWithLabel
                        title="رونوشت"
                        text={
                          regardingData.isCopy ? regardingData.isCopy : "ندارد"
                        }
                      />
                    </div>
                    <div className="sm:col-span-2">
          ...
          <div className="fixed inset-0  flex items-start justify-center p-2">
            ...
            >
              {previewLoading ? (
                ...
                >
                  <Image src="/images/dots.svg" height={22} width={22} />
                </div>
              ) : imageContent ? (
                <Image
